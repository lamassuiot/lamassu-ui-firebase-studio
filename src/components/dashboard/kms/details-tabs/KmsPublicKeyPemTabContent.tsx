...
export const KmsPublicKeyPemTabContent: React.FC<KmsPublicKeyPemTabContentProps> = ({
  ...
  const handleCopyPublicKey = async () => {
    ...
    try {
      await navigator.clipboard.writeText(publicKeyPem.replace(/\\n/g, '\n')); // Ensure newlines are actual newlines
    ...
    <div className="space-y-6 py-4">
      <div>
        <div className="flex justify-between items-center mb-2">
          ...
          <Button onClick={handleCopyPublicKey} variant="outline" size="sm" disabled={!publicKeyPem}>
        ...
        {publicKeyPem ? (
          <ScrollArea className="h-80 w-full rounded-md border p-3 bg-muted/30">
            <pre className="text-xs whitespace-pre-wrap break-all font-mono">{publicKeyPem.replace(/\\n/g, '\n')}</pre>
          </ScrollArea>
        ) : (
