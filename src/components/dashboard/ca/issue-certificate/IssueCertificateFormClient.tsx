...
function formatPkijsSans(extensions: PkijsExtension[]): string[] {
  ...
  if (sanExtension && sanExtension.parsedValue) {
      (sanExtension.parsedValue as PkijsGeneralNames).names.forEach(name => {
          if (name.type === 2) sans.push(`DNS: ${name.value}`);
          else if (name.type === 7) {
...
export default function IssueCertificateFormClient() {
  ...
  const [signAsCa, setSignAsCa] = useState(false);
  ...
  const handleGenerateAndReview = async () => {
    ...
    try {
      ...
      dnsSans.split(',').filter(s => s.trim()).forEach(dnsName => generalNamesArray.push(new GeneralName({ type: 2, value: dnsName.trim() })));
      ipSans.split(',').filter(s => s.trim()).forEach(ip => {
        ...
        <CardContent>
            ...
            <div className="space-y-6 mt-6">
                ...
                {issuanceMode === 'generate' ? (
                  <div className="space-y-4">
                    ...
                    <h3 className="font-medium text-lg pt-4">Subject Alternative Names (Optional)</h3>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1"><Label htmlFor="dnsSans">DNS Names (comma-separated)</Label><Input id="dnsSans" value={dnsSans} onChange={e => setDnsSans(e.target.value)} /></div>
                        <div className="space-y-1"><Label htmlFor="ipSans">IP Addresses (comma-separated)</Label><Input id="ipSans" value={ipSans} onChange={e => setIpSans(e.target.value)} /></div>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-4">
          ...
          {step === 3 && (
             <div className="space-y-6 mt-6">
                ...
                <div className="flex items-center space-x-2"><Switch id="signAsCa" checked={signAsCa} onCheckedChange={setSignAsCa} /><Label htmlFor="signAsCa">Sign as a CA (Basic Constraints: CA:TRUE)</Label></div>
