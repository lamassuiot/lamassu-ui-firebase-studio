...
export default function EditRegistrationAuthorityPage() {
  const router = useRouter();
  const params = useParams();
  const raId = Array.isArray(params.raId) ? params.raId[0] : params.raId;
  const { user, isLoading: authLoading, isAuthenticated } = useAuth();
  const { toast } = useToast();
  
  const [raData, setRaData] = useState<ApiRaItem | null>(null);
  const [isLoadingRA, setIsLoadingRA] = useState(true);
  ...
  const loadDependencies = useCallback(async () => {
    ...
    try {
        ...
        setErrorCAsForSelection(err.message || 'Failed to load CAs');
        setErrorEngines(err.message || 'Failed to load engines');
    } finally {
  ...
  const fetchRaDetails = useCallback(async () => {
    if (!raId || !isAuthenticated() || !user?.access_token) return;

    setIsLoadingRA(true);
    setErrorRA(null);
    try {
        const response = await fetch(`https://lab.lamassu.io/api/dmsmanager/v1/dms/${raId}`, {
            headers: { 'Authorization': `Bearer ${user.access_token}` },
        });
        if (!response.ok) {
             let errorJson;
  ...
  }, [raId, user?.access_token, isAuthenticated]);
  ...
  }, [authLoading, loadDependencies, fetchRaDetails]);

  // Effect to populate form once both RA data and CA list are available
  useEffect(() => {
    if (raData && availableCAsForSelection.length > 0) {
        ...
        // Populate re-enrollment settings
        ...
        // Populate server keygen settings
        ...
        // Populate CA distribution settings
  ...
  }, [raData, availableCAsForSelection]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        ...
        if (!response.ok) {
            let errorJson;
            let errorMessage = `Failed to update RA. Status: ${response.status}`;
            try {
                errorJson = await response.json();
                errorMessage = `RA update failed: ${errorJson.err || errorJson.message || 'Unknown error'}`;
            } catch (e) { /* ignore */ }
            throw new Error(errorMessage);
        }
        
        toast({ title: "Success!", description: `Registration Authority "${raName}" updated successfully.` });
      ...
      <>
        ...
        {errorCAsForSelection && !isLoadingCAsForSelection && !authLoading && (
          <Alert variant="destructive" className="my-4">
            ...
            <AlertDescription>
              {errorCAsForSelection} <Button variant="link" onClick={loadDependencies} className="p-0 h-auto">Try again?</Button>
            </AlertDescription>
          </Alert>
        )}
            ...
            <ul className="space-y-0.5 p-2">
              {availableCAsForSelection.map((ca) => (
                <SelectableCaTreeItem 
                  ...
                  onMultiSelectToggle={toggleCaSelectionInList}
      ...
      return (
          <div className="w-full space-y-4 p-4">
              ...
              <Alert variant="destructive">
                  <AlertTriangle className="h-4 w-4" />
                  <AlertTitle>Error Loading RA</AlertTitle>
                  <AlertDescription>{errorRA}</AlertDescription>
              </Alert>
          </div>
      );
  }
  ...
  const SelectedIconComponent = getReactIconByName(selectedDeviceIconName);
  ...
  return (
    <div className="w-full space-y-6 mb-8">
      ...
      <div> 
        <div className="flex flex-col space-y-1.5 p-6"> 
          <h1 className="text-xl font-headline flex items-center font-semibold leading-none tracking-tight"> 
            <Edit className="mr-2 h-6 w-6 text-primary" /> Edit Registration Authority
          </h1>
        ...
        <div className="p-6 pt-0"> 
          <form onSubmit={handleSubmit}>
              ...
              <CardContent className="p-4">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input id="raId" value={raId} readOnly disabled className="mt-1 bg-muted/50" />
                  </div>
                </div>
              </CardContent>
            </Card>
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-2">
                    ...
                    <Button id="deviceIconButton" type="button" variant="outline" onClick={() => setIsDeviceIconModalOpen(true)} className="w-full justify-start text-left font-normal flex items-center gap-2 mt-1">
                        {SelectedIconComponent ? <div className="flex items-center gap-2"><div className="p-1 rounded-sm flex items-center justify-center" style={{ backgroundColor: selectedDeviceIconBgColor }}><SelectedIconComponent className="h-5 w-5" style={{ color: selectedDeviceIconColor }} /></div>{selectedDeviceIconName}</div> : "Select Device Icon..."}
                    </Button>
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="border-border shadow-sm rounded-md">
              <CardContent className="p-4">
                <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-2 pt-2"><Switch id="allowOverrideEnrollment" checked={allowOverrideEnrollment} onCheckedChange={setAllowOverrideEnrollment} /><Label htmlFor="allowOverrideEnrollment">Allow Override Enrollment</Label></div>
                  <div>
                    <Label htmlFor="authMode">Authentication Mode</Label>
                    <Select value={authMode} onValueChange={setAuthMode}><SelectTrigger id="authMode" className="mt-1"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Client Certificate">Client Certificate</SelectItem><SelectItem value="External Webhook">External Webhook</SelectItem><SelectItem value="No Auth">No Auth</SelectItem></SelectContent></Select>
                  </div>
                  <div>
                    <Label htmlFor="validationCAs">Validation CAs</Label>
                    <Button type="button" variant="outline" onClick={() => setIsValidationCaModalOpen(true)} className="w-full justify-start text-left font-normal mt-1" disabled={isLoadingCAsForSelection || authLoading}>{isLoadingCAsForSelection || authLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : validationCAs.length > 0 ? `Selected ${validationCAs.length} CA(s) - Click to modify` : "Select Validation CAs..."}</Button>
                    {validationCAs.length > 0 && <div className="mt-2 flex flex-wrap gap-2">{validationCAs.map(ca => <CaVisualizerCard key={ca.id} ca={ca} className="shadow-none border-border max-w-xs" allCryptoEngines={allCryptoEngines} />)}</div>}
                  </div>
                  ...
                  <div>
                    <Label htmlFor="chainValidationLevel" className="flex items-center">Chain Validation Level<TooltipProvider><Tooltip><TooltipTrigger asChild><HelpCircle className="ml-1 h-4 w-4 text-muted-foreground cursor-help" /></TooltipTrigger><TooltipContent><p>-1 equals full chain validation.</p></TooltipContent></Tooltip></TooltipProvider></Label>
            ...
            <Card className="border-border shadow-sm rounded-md"><CardContent className="p-4"><div className="space-y-4"><div className="flex items-center space-x-2"><Switch id="revokeOnReEnroll" checked={revokeOnReEnroll} onCheckedChange={setRevokeOnReEnroll} /><Label htmlFor="revokeOnReEnroll">Revoke On Re-Enroll</Label></div><div className="flex items-center space-x-2"><Switch id="allowExpiredRenewal" checked={allowExpiredRenewal} onCheckedChange={setAllowExpiredRenewal} /><Label htmlFor="allowExpiredRenewal">Allow Expired Renewal</Label></div>
            <div><Label htmlFor="allowedRenewalDelta">Allowed Renewal Delta (e.g., 100d)</Label><Input id="allowedRenewalDelta" value={allowedRenewalDelta} onChange={(e) => setAllowedRenewalDelta(e.target.value)} placeholder="e.g., 100d" className="mt-1" /></div>
            <div><Label htmlFor="preventiveRenewalDelta">Preventive Renewal Delta (e.g., 31d)</Label><Input id="preventiveRenewalDelta" value={preventiveRenewalDelta} onChange={(e) => setPreventiveRenewalDelta(e.target.value)} placeholder="e.g., 31d" className="mt-1" /></div>
            <div><Label htmlFor="criticalRenewalDelta">Critical Renewal Delta (e.g., 7d)</Label><Input id="criticalRenewalDelta" value={criticalRenewalDelta} onChange={(e) => setCriticalRenewalDelta(e.target.value)} placeholder="e.g., 7d" className="mt-1" /></div>
            <div><Label htmlFor="additionalValidationCAs">Additional Validation CAs (for re-enrollment)</Label><Button type="button" variant="outline" onClick={() => setIsAdditionalValidationCaModalOpen(true)} className="w-full justify-start text-left font-normal mt-1" disabled={isLoadingCAsForSelection || authLoading}>{isLoadingCAsForSelection || authLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : additionalValidationCAs.length > 0 ? `Selected ${additionalValidationCAs.length} CA(s)` : "Select CAs..."}</Button>
            {additionalValidationCAs.length > 0 && <div className="mt-2 flex flex-wrap gap-2">{additionalValidationCAs.map(ca => <CaVisualizerCard key={ca.id} ca={ca} className="shadow-none border-border max-w-xs" allCryptoEngines={allCryptoEngines}/>)}</div>}</div></div></CardContent></Card>
      ...
      <CaSelectorModal isOpen={isEnrollmentCaModalOpen} onOpenChange={setIsEnrollmentCaModalOpen} title="Select Enrollment CA" description="Choose the CA that will issue certificates." availableCAs={availableCAsForSelection} isLoadingCAs={isLoadingCAsForSelection} errorCAs={errorCAsForSelection} loadCAsAction={loadDependencies} onCaSelected={(ca) => { setEnrollmentCa(ca); setIsEnrollmentCaModalOpen(false); }} currentSelectedCaId={enrollmentCa?.id} isAuthLoading={authLoading} allCryptoEngines={allCryptoEngines} />
      <CaSelectorModal isOpen={isValidationCaModalOpen} onOpenChange={setIsValidationCaModalOpen} title="Select Validation CAs" description="Choose CAs to validate client certificates." availableCAs={availableCAsForSelection} isLoadingCAs={isLoadingCAsForSelection} errorCAs={errorCAsForSelection} loadCAsAction={loadDependencies} onCaSelected={() => {}} isAuthLoading={authLoading} allCryptoEngines={allCryptoEngines}>{renderMultiSelectCaDialogContent(validationCAs, setValidationCAs, setIsValidationCaModalOpen)}</CaSelectorModal>
      <CaSelectorModal isOpen={isAdditionalValidationCaModalOpen} onOpenChange={setIsAdditionalValidationCaModalOpen} title="Select Additional Validation CAs" description="Choose CAs for re-enrollment validation." availableCAs={availableCAsForSelection} isLoadingCAs={isLoadingCAsForSelection} errorCAs={errorCAsForSelection} loadCAsAction={loadDependencies} onCaSelected={() => {}} isAuthLoading={authLoading} allCryptoEngines={allCryptoEngines}>{renderMultiSelectCaDialogContent(additionalValidationCAs, setAdditionalValidationCAs, setIsAdditionalValidationCaModalOpen)}</CaSelectorModal>
      <CaSelectorModal isOpen={isManagedCaModalOpen} onOpenChange={setIsManagedCaModalOpen} title="Select Managed CAs" description="Choose CAs for the CA certs endpoint." availableCAs={availableCAsForSelection} isLoadingCAs={isLoadingCAsForSelection} errorCAs={errorCAsForSelection} loadCAsAction={loadDependencies} onCaSelected={() => {}} isAuthLoading={authLoading} allCryptoEngines={allCryptoEngines}>{renderMultiSelectCaDialogContent(managedCAs, setManagedCAs, setIsManagedCaModalOpen)}</CaSelectorModal>
