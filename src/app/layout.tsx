...
const InnerLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  if (mounted && isAuthenticated() && user?.access_token) { // Check mounted here
    try {
  ...
  const LoadingState = () => (
    <div className="flex flex-1 flex-col items-center justify-center p-6 text-center">
      ...
      <p className="mt-6 text-lg text-muted-foreground">
        {(mounted && isLoading) ? "Loading authentication status..." : "Loading application..."}
      </p>
    </div>
  );
  ...
  return (
    <SidebarProvider defaultOpen>
      <div className="flex flex-col h-screen bg-background text-foreground w-full">
        {(!mounted || isLoading) ? <LoadingState /> : <MainLayoutContent />}
      </div>
    </SidebarProvider>
  );
};
